...
$totalSubjects = !empty($data['subjects']) ? count($data['subjects']) : 0;
$totalTeached = 0;

if (!empty($data['subjects'])) {
    foreach ($data['subjects'] as $subject) {
        if (!empty($subject['teacher_count']) || !empty($subject['teacher_name'])) {
            $totalTeached++;
        }
    }
}
...
<main 
    ...
    <div class="p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        <!-- DASHBOARD WIDGET STATISTIK MATA PELAJARAN -->
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 lg:gap-6">
            ...
            <div class="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-100 dark:border-slate-700/80 shadow-xs flex items-center justify-between transition-colors">
                <div class="space-y-1">
                    <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Total Mapel</span>
                    <h3 class="text-2xl sm:text-3xl font-extrabold text-slate-800 dark:text-white leading-tight">
                        <?= $totalSubjects ?> <span class="text-xs font-semibold text-slate-400">Mapel</span>
                    </h3>
                </div>
            ...
            <!-- CARD 2: MAPEL MEMILIKI PENGAJAR -->
            <div class="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-100 dark:border-slate-700/80 shadow-xs flex items-center justify-between transition-colors">
                <div class="space-y-1">
                    <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Mapel Terisi Pengajar</span>
                    <h3 class="text-2xl sm:text-3xl font-extrabold text-emerald-600 dark:text-emerald-400 leading-tight">
                        <?= $totalTeached ?> <span class="text-xs font-semibold text-slate-400">Mapel</span>
                    </h3>
                </div>
            ...
            <!-- CARD 3: MAPEL BELUM ADA PENGAJAR -->
            <div class="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-100 dark:border-slate-700/80 shadow-xs flex items-center justify-between transition-colors">
                <div class="space-y-1">
                    <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Belum Ada Pengajar</span>
                    <h3 class="text-2xl sm:text-3xl font-extrabold text-amber-500 leading-tight">
                        <?= max(0, $totalSubjects - $totalTeached) ?> <span class="text-xs font-semibold text-slate-400">Mapel</span>
                    </h3>
                </div>
                <div class="w-12 h-12 rounded-2xl bg-amber-50 dark:bg-amber-950/40 text-amber-500 flex items-center justify-center shrink-0">
        ...
        <!-- CONTAINER UTAMA (DATA MATA PELAJARAN) -->
                    ...
                    <table class="w-full text-center border-collapse">
                        <thead>
                            <tr class="bg-slate-50/80 dark:bg-slate-700/40 border-b border-slate-200/80 dark:border-slate-700/80 text-[11px] font-bold text-slate-400 dark:text-slate-400 uppercase tracking-wider">
                                ...
                                <th class="px-6 py-3.5 text-left">Nama Mata Pelajaran</th>
                            ...
                            <?php if (!empty($data['subjects'])): ?>
                                <?php foreach ($data['subjects'] as $item): ?>
                                    <tr 
                                        ...
                                        x-show="search === '' || 
                                                '<?= strtolower(addslashes($item['subject_name'])) ?>'.includes(search.toLowerCase())"
                                    >
                                        ...
                                        <td class="px-4 py-4">
                                            <div class="flex items-center justify-center gap-1">
                                                ...
                                                <button
                                                    @click="
                                                        subjects = {id: '<?= $item['id'] ?>', subject_name: '<?= addslashes($item['subject_name']) ?>'}; 
                                                        modaledit = true;
                                                    "
                                                    type="button" 
    ...
    <div 
        ...
        <form action="<?= base_url('/admin/mata-pelajaran') ?>" method="post" class="w-full max-w-lg">
            <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl border border-slate-100 dark:border-slate-700/80 transition-colors" @click.away="modaladd = false">
                <div class="flex justify-between items-center pb-4 border-b border-slate-100 dark:border-slate-700/60 mb-6">
                    <h6 class="font-bold text-base sm:text-lg text-slate-800 dark:text-white">Tambah Mata Pelajaran</h6>
                ...
                <div class="space-y-4 mb-8">
                    <div>
                        <label class="text-slate-700 dark:text-slate-300 text-xs font-bold uppercase tracking-wider block mb-2">Nama Mata Pelajaran</label>
                        <input required name="subject_name" type="text" class="border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700/50 text-slate-800 dark:text-slate-100 w-full rounded-xl py-2.5 px-4 text-xs sm:text-sm focus:outline-none focus:bg-white dark:focus:bg-slate-700 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all font-normal" placeholder="Contoh: Matematika, Bahasa Indonesia...">
                    </div>
                </div>
    ...
    <div 
        ...
        <form :action="'<?= base_url('/admin/mata-pelajaran') ?>/' + subjects.id" method="post" class="w-full max-w-lg">
            <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl border border-slate-100 dark:border-slate-700/80 transition-colors" @click.away="modaledit = false">
                <div class="flex justify-between items-center pb-4 border-b border-slate-100 dark:border-slate-700/60 mb-6">
                    <div>
                        ...
                        <h6 class="font-bold text-base sm:text-lg text-slate-800 dark:text-white" x-text="subjects.subject_name"></h6>
                    </div>
                ...
                <div class="space-y-4 mb-8">
                    <div>
                        <label class="text-slate-700 dark:text-slate-300 text-xs font-bold uppercase tracking-wider block mb-2">Nama Mata Pelajaran</label>
                        <input x-model="subjects.subject_name" required name="subject_name" type="text" class="border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700/50 text-slate-800 dark:text-slate-100 w-full rounded-xl py-2.5 px-4 text-xs sm:text-sm focus:outline-none focus:bg-white dark:focus:bg-slate-700 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all font-normal" placeholder="Contoh: Matematika...">
                    </div>
                </div>
    ...
    <div 
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 sm:p-8 max-w-md w-full shadow-xl border border-slate-100 dark:border-slate-700/80 transition-colors" @click.away="modaldel = false">
            <div class="text-center mb-6">
                ...
                <h6 class="text-base sm:text-lg font-bold text-slate-800 dark:text-white mb-2">Hapus Mata Pelajaran?</h6>
                <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400">
                    Apakah Anda yakin ingin menghapus <span x-text="subjects.subject_name" class="font-bold text-slate-800 dark:text-white"></span>?
                </p>
            </div>

            <div class="flex items-center gap-3">
                <a :href="'<?= base_url('/admin/mata-pelajaran/del') ?>/' + subjects.id" class="flex-1 text-center font-bold cursor-pointer bg-gradient-to-r from-rose-600 to-red-500 rounded-xl text-white py-2.5 px-4 hover:from-rose-700 hover:to-red-600 transition-all duration-300 shadow-md hover:shadow-lg active:scale-[0.98] text-xs sm:text-sm">
                    Ya, Hapus
                </a>
